...
#bar_graph_dem <- growth_democracy %>% filter(year == 1984) %>% ggplot()
```


```{r, Graphs on Life Expectancy}
afghanistan_life <- life_expectancy %>% filter(entity == "Afghanistan")

afghanistan_life_graph <- afghanistan_life %>% ggplot(aes(x = year, y = life_expectancy_years)) + 
                               geom_line(color = "red") + ggtitle("Afgani Life Expectancy") + labs(x = "Year", y = "Expected Age of Death") + theme_igray() + geom_vline(xintercept = 1988, color = "blue")

sweden_le <- life_expectancy %>% filter(entity == "Sweden") %>% filter(year > 1890)
sweden_le_graph <- sweden_le %>% ggplot(aes(x = year, y = life_expectancy_years)) + geom_line(color = "red") + ggtitle("Swedish Life Expectancy") + labs(x = "Year", y = "Expected Age of Death") + theme_igray() + geom_vline(xintercept = 1920, color = "blue")

indonesia_le <- life_expectancy %>% filter(entity == "Indonesia") %>% filter(year > 1960)
indonesia_le_graph <- indonesia_le %>% ggplot(aes(x = year, y = life_expectancy_years)) + geom_line(color = "red") + ggtitle("Indonesian Life Expectancy") + labs(x = "Year", y = "Expected Age of Death") + theme_igray() + geom_vline(xintercept = 1999, color = "blue")
```
